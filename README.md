...
*   **Warm-up (Разогрев):** Перед каждым замером выполняется серия итераций алгоритма «вхолостую». Это позволяет прогреть кэши процессора (L1/L2), заполнить таблицы предсказателя переходов и стабилизировать частоту CPU.
